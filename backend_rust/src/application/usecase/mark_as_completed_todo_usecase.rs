...
pub struct MarkAsCompletedTodoUsecase<TS>
where
...
impl<TS> MarkAsCompletedTodoUsecase<TS>
where
    TS: TransactionService,
{
            ...
            .run(move || {
                ...
                async move {
                    ...
                    todo.mark_as_completed()
                        .map_err(UsecaseError::conflict_from_already_completed)?;
...
mod tests {
    ...
    async fn mark_completed_sets_flag() {
        ...
        let usecase = MarkAsCompletedTodoUsecase::new(repository.clone(), transaction_service);
        let dto = usecase.execute(&id).await.expect("mark completed");
