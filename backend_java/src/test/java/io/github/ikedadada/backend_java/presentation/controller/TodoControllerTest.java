...
class TodoControllerTest {
    ...
    void createTodoReturnsCreatedTodo() throws Exception {
        ...
        when(createTodoUsecase.handle(eq("new todo"), eq("write presentation tests"))).thenReturn(created);
        ...
        mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post("/todos")
                ...
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$.completed")
                        .value(false));

        verify(createTodoUsecase).handle("new todo", "write presentation tests");
    }
    ...
    void getTodosReturnsTodosFromUsecase() throws Exception {
        ...
        mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get("/todos")
                ...
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$.todos",
                        hasSize(2)))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$.todos[*].id",
    ...
    void updateTodoReturnsUpdatedTodo() throws Exception {
        ...
        when(updateTodoUsecase.handle(eq(id), eq("updated"), eq("new description"))).thenReturn(updated);
    ...
    void updateTodoReturnsNotFoundWhenMissing() throws Exception {
        UUID id = UUID.randomUUID();
        when(updateTodoUsecase.handle(eq(id), eq("title"), any())).thenThrow(new RepositoryException.TodoNotFound(id.toString()));
    ...
    void markAsCompletedReturnsConflictWhenAlreadyCompleted() throws Exception {
        ...
        when(markAsCompletedTodoUsecase.handle(id)).thenThrow(new DomainException.TodoAlreadyCompleted(existing));
    ...
    void markAsNotCompletedReturnsConflictWhenNotCompleted() throws Exception {
        ...
        when(markAsNotCompletedTodoUsecase.handle(id)).thenThrow(new DomainException.TodoNotCompleted(existing));
